...
def DirectoryScanner(DirectoryPath):
    ...
    LogFileName="Marvellous %s.log" %(timestamp)   # %s =String
    LogFileName=LogFileName.replace(" ","_")
    LogFileName=LogFileName.replace(":","_")

    Ret=False

    # Filter 1 --> checks if Directory name exists 
    Ret=os.path.exists(DirectoryPath)
    if Ret == False:
        print("Marvellous Automation Error : There is no such Directory with name ",DirectoryPath)
        return
    
    # Filter 2 --> checks if provided is directory
...
def main():
    ...
    if(len(sys.argv) ==2 ):
        if(sys.argv[1] == "--h" or sys.argv[1] =="--H"):
            ...
            print("python FileName.py DirectoryNmae")
            print("DirectioryName should be in ABSOLUTE Path")
